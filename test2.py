...
class HelicoilDepthCheck:
    ...
    def _draw_surface(self, frame: np.ndarray, detections, surface_index: int, hand_coords_list: list[list[int]]):
        ...
        if self.fin_coordinates is not None:
            for hand_coords in hand_coords_list:
                ...
                if len(distances_to_fin) > 0:
                    ...
                    if min_hand_fin_distance < self.driver_hand_thresh:
                        hand_far_from_fin = False
                        print("Hand is too close to the fin. Skipping surface visualization.")
                        break
        
        # If hand is far from the fin, don't visualize the surface
        if hand_far_from_fin and surface_index is not None:
            ...
            cv2.polylines(frame, [self.surface_coordinates.astype(np.int32)], isClosed=True, color=(0, 255, 255), thickness=2)
            print("Surface visualized.")
        elif self.previous_box is not None:
            # Draw the previous box if conditions not met for a new one
            print("Persisting previous surface visualization.")
            cv2.polylines(frame, [self.previous_box.astype(np.int32)], isClosed=True, color=(0, 255, 255), thickness=2)
        else:
            print("No surface detected or conditions not met.")
    ...
    def _check_operator(self, frame: np.ndarray, timestamp: float):
        ...
        self._draw_surface(frame, detections, surface_index, hand_coords_list)
